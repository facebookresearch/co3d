...
def is_known_frame(frame_type: List[str], device: Optional[str] = None) -> torch.Tensor:
    """
...
def is_train_frame(frame_type: List[str], device: Optional[str] = None) -> torch.Tensor:
    """
    Given a list `frame_type` of frame types in a batch, return a tensor
